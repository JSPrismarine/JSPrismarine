...
export default class BatchPacket extends DataPacket {
  ...
  public decodePayload(): void {
    this.payload.write(
      this.compressed
        ? CompressionProvider.fromAlgorithmSync(this.readByte())(
            this.readRemaining(),
          )
        : this.readRemaining(),
    );
  ...
  public async asyncDecode(): Promise<Buffer[]> {
    ...
    try {
      this.payload.write(
        this.compressed
          ? await CompressionProvider.fromAlgorithm(this.readByte())(
              this.readRemaining(),
            )
          : this.readRemaining(),
      );
      ...
      throw new Error(
        `Failed to inflate batched content (${(<Error>e).message})`,
      );
    }
  ...
  public encodePayload(): void {
    ...
    this.write(
      this.compressed
        ? Zlib.deflateRawSync(this.payload.getBuffer(), { level: 7 })
