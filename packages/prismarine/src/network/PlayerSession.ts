...
export default class PlayerSession {
  ...
  public async update(_tick: number): Promise<void> {
    if (this.chunkSendQueue.length > 0) {
      const chunksToSend = this.chunkSendQueue.splice(
        0,
        Math.min(this.chunkSendQueue.length, 50),
      );
  ...
  public async sendAbilities(player?: Player): Promise<void> {
    ...
    mainLayer.layerFlags = new Map([
      ...
      [
        AbilityLayerFlag.DOORS_AND_SWITCHES,
        target.gamemode !== Gamemode.Spectator,
      ],
      [
        AbilityLayerFlag.OPEN_CONTAINERS,
        target.gamemode !== Gamemode.Spectator,
      ],
  ...
  public async needNewChunks(
    ...
    for (
      ...
      for (
        ...
        if (
          sendXChunk * sendXChunk + sendZChunk * sendZChunk >
          viewDistance * viewDistance
        )
          continue; // early exit if chunk is outside of view distance
        const newChunk = [
      ...
      if (forceResend) {
        if (!this.loadedChunks.has(hash) && !this.loadingChunks.has(hash)) {
          ...
          const loadedChunk = await this.player
            .getWorld()
            .getChunk(closestChunk[0]!, closestChunk[1]!);
    ...
    creativeitems.forEach((item: any) => {
      pk.items.push(
        ...entries
          .filter((entry: any) => {
            return (
              entry.meta === (item.damage ?? 0) && entry.getId() === item.id
            );
          })
          .map(
            (entry) =>
      ...
      .forEach((command) => {
        ...
        if (
          !this.player
            ...
            .can(this.player)
            .execute(commandClass.permission)
        )
          return;
  ...
  public async removeFromPlayerList(): Promise<void> {
    ...
    await Promise.all(
      this.server
        ...
        .getAllPlayers()
        .map(async (player) =>
          player.getNetworkSession().getConnection().sendDataPacket(pk),
        ),
    );
  }
  ...
  public async sendPlayerList(): Promise<void> {
    const playerList = new PlayerListPacket();
    playerList.type = PlayerListAction.TYPE_ADD;
    playerList.entries = Array.from(
      this.server.getSessionManager().getPlayerList().values(),
    );
