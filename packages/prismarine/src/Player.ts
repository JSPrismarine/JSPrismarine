...
export default class Player extends Human {
  ...
  public async onEnable() {
    ...
    playerData.inventory.forEach((item) => {
      ...
      if (!entry) {
        this.getServer()
          .getLogger()
          ?.warn(`Item/block with id ${item.id} is invalid`, "Player/onEnable");
        return;
      }
    ...
    if (!this.connected)
      this.getServer()
        .getLogger()
        ?.debug(
          `(Complete player creation took ${this.joinTimer.stop()} ms)`,
          "Player/onEnable",
        );
    this.connected = true;
  }
  ...
  public async sendInitialSpawnChunks(): Promise<void> {
    const minX =
      CoordinateUtils.fromBlockToChunk(this.x) - VANILLA_DEFAULT_SPAWN_RADIUS;
    const minZ =
      CoordinateUtils.fromBlockToChunk(this.z) - VANILLA_DEFAULT_SPAWN_RADIUS;
    const maxX =
      CoordinateUtils.fromBlockToChunk(this.x) + VANILLA_DEFAULT_SPAWN_RADIUS;
    const maxZ =
      CoordinateUtils.fromBlockToChunk(this.z) + VANILLA_DEFAULT_SPAWN_RADIUS;
    ...
    await this.networkSession.sendNetworkChunkPublisher(
      VANILLA_DEFAULT_SPAWN_RADIUS,
      savedChunks,
    );
  ...
  public async update(tick: number): Promise<void> {
    ...
    for (const chunk of this.chunkSendQueue) {
      await this.networkSession.sendNetworkChunkPublisher(
        this.viewDistance ?? VANILLA_DEFAULT_SPAWN_RADIUS,
        [],
      );
      await this.networkSession.sendChunk(chunk);
  ...
  public async kick(reason = "unknown reason"): Promise<void> {
    this.getServer()
      .getLogger()
      ?.verbose(
        `Player with id §b${this.getRuntimeId()}§r was kicked: ${reason}`,
        "Player/kick",
      );
